...
def generate_spi_mcp2515_sample():
    """Generate SPI Transaction (Write CANCTRL) - Enhanced for detection"""
    ...
    with open(filepath, 'w', newline='') as f:
        ...
        # Helper untuk write 1 bit SPI dengan clock cycles yang cukup
        def write_spi_bit(mosi_val, miso_val):
            nonlocal current_ns
            # Clock Low
        ...
        # Transaction 1: Command Byte 0x02 (Write)
        ...
        # --- TRANSACTION 2 (Repeat for more edges) ---
        # Idle time
        for _ in range(100):
            write_row(f, current_ns, 1, 0, 1, 1, 1, 1, 1, 1)
            current_ns += SAMPLE_PERIOD_NS
        ...
        # CS Pull Down again
        for _ in range(50):
            write_row(f, current_ns, 0, 0, 1, 1, 1, 1, 1, 1)
            current_ns += SAMPLE_PERIOD_NS
        
        # Transaction 2: Different data pattern
        for bit in [0,0,0,0,0,0,1,0]:  # Command
            write_spi_bit(bit, 1)  # MISO responds with 1
        for bit in [0,0,1,0,1,0,1,0]:  # Address 0x2A (CNF1)
            write_spi_bit(bit, 1)
        for bit in [0,1,0,0,0,0,1,1]:  # Data 0x43
            write_spi_bit(bit, 1)
